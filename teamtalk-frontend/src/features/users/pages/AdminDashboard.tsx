...
const ROLES: { value: UserRole; label: string; color: string }[] = [
  { value: 'director', label: 'Director', color: '#8b5cf6' },
...
const RoleDropdown: React.FC<RoleDropdownProps> = ({ user, onUpdate }) => {
  ...
  const current = ROLES.find((r) => r.value === user.role) ?? ROLES[ROLES.length - 1];
...
const AdminDashboard: React.FC = () => {
  ...
  const roleData = [
    { name: 'Director', value: users.filter((u) => u.role === 'director').length, color: '#8b5cf6' },
