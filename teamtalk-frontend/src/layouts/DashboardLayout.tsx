...
import { cn, resolveNotificationTarget, getUserRoles, hasRole } from '@/utils';
...
const Sidebar: React.FC = () => {
  ...
  const extraRolesCount = userRoles.length > 1 ? userRoles.length - 1 : 0;

  const adminNav = (isAdmin || isDirector)
    ? [
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-white/70 dark:bg-white/10 border border-subtle overflow-hidden">
          <img
            src={slogo}
            alt="Team Talk"
            className={cn('object-contain', collapsed ? 'w-8 h-8' : 'w-7 h-7')}
          />
        </div>
        {!collapsed && (
          <div className="min-w-0 flex-1">
            <div className="font-bold text-sm text-gray-900 dark:text-gray-100 font-display">Team Talk</div>
        ...
        {title ? (
          ...
          <>
            <div className="flex items-center gap-1.5 font-bold text-[15px] text-gray-900 dark:text-gray-100 font-display">
              {channel.isVoice ? <Volume2 size={15} /> : channel.isPrivate ? <Lock size={15} /> : <Hash size={15} />}
              {channel.name}
            </div>
            ...
            <div className="max-h-72 overflow-y-auto scrollbar-thin">
              {notifications.map((n) => (
                <NotificationItem
                  ...
                  compact
                  onClick={(notif) => {
                    if (!notif.read) dispatch(markAsReadApi(notif.id));
                    const target = resolveNotificationTarget(notif, channels);
...
export const DashboardLayout: React.FC = () => {
  ...
  const [groupCall, setGroupCall] = useState<{
    channelId: string; callerId: string; callerName: string; groupName?: string;
  } | null>(null);
  const dismissTimerRef = React.useRef<number | null>(null);
  const stopGroupRingRef = React.useRef<(() => void) | null>(null);
  ...
  useEffect(() => {
    ...
    // Register AFTER start() resolves so we run after useSignalR's offAll() cleanup
    signalRService.start().then(() => {
      signalRService.onIncomingCall(onIncoming);
    });
  ...
  useEffect(() => {
    const onVoiceJoined = (e: Event) => {
      ...
      const joinedSet = (window as unknown as { __TeamTalkVoiceChannels?: Set<string> }).__TeamTalkVoiceChannels;
      if (joinedSet?.has(channelId)) return;
      const user = allUsersRef.current.find(u => u.id === userId);
      const callerName = user?.name ?? 'Quelqu\'un';
      setGroupCall({ channelId, callerId: userId, callerName, groupName });
      if (dismissTimerRef.current) clearTimeout(dismissTimerRef.current);
      dismissTimerRef.current = window.setTimeout(() => setGroupCall(null), 20000);
    };
    window.addEventListener('Team Talk:voiceJoined', onVoiceJoined);
    return () => {
      window.removeEventListener('Team Talk:voiceJoined', onVoiceJoined);
      if (dismissTimerRef.current) clearTimeout(dismissTimerRef.current);
    };
  }, [me?.id]);

  // Group call ringing sound
  useEffect(() => {
    if (groupCall) {
      const isChannel = !groupCall.groupName || !groupCall.groupName.startsWith('GROUPE:');
      stopGroupRingRef.current = createRingtone(isChannel ? 'channel' : 'group');
      return () => { stopGroupRingRef.current?.(); stopGroupRingRef.current = null; };
    }
  }, [!!groupCall]);

  const acceptGroupCall = () => {
    if (!groupCall) return;
    stopGroupRingRef.current?.(); stopGroupRingRef.current = null;
    const channelId = groupCall.channelId;
    setGroupCall(null);
    if (dismissTimerRef.current) clearTimeout(dismissTimerRef.current);
    navigate('/dashboard/messages', { state: { joinGroupChannelId: channelId } });
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-surface-50 dark:bg-surface-950">
      ...
      {/* Group / Channel voice call — compact toast notification (top-right) */}
      <AnimatePresence>
        {groupCall && (
          <motion.div
          ...
          >
            <div className="flex items-center gap-3">
              <div className="relative flex-shrink-0">
                ...
                {(() => {
                  const caller = allUsers.find(u => u.id === groupCall.callerId);
                  return caller
                    ? <Avatar user={caller} size="md" showStatus={false} />
                    : <div className="w-10 h-10 rounded-full bg-brand-500/10 flex items-center justify-center"><Users size={18} className="text-brand-500" /></div>;
                })()}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-gray-900 dark:text-gray-100 text-sm truncate">{groupCall.callerName}</div>
                <div className="text-xs text-gray-400 truncate flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block" />
                  {groupCall.groupName ? groupCall.groupName : 'Canal vocal'}
                </div>
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={rejectGroupCall} title="Ignorer"
                ...
                <button onClick={acceptGroupCall} title="Rejoindre"
