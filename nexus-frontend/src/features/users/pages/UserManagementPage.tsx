...
const ROLES: { value: UserRole; label: string; color: string; desc: string }[] = [
  ...
  { value: 'manager',  label: 'Manager',  color: '#6247ea', desc: 'Manage teams, members & channels' },
...
// highest role wins when multiple selected
...
const RoleSelect: React.FC<{
  ...
  onChange: (primary: UserRole, secondary: UserRole | null) => void;
}> = ({ value, secondary, onChange }) => {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<Set<UserRole>>(
    () => new Set(secondary ? [value, secondary] : [value])
  );
  ...
  const handleApply = () => {
    const arr = [...selected] as UserRole[];
    const prim = primaryRole(arr);
    const sec  = arr.find((x) => x !== prim) ?? null;
    onChange(prim, sec);
    setOpen(false);
  };
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-md hover:opacity-80 transition-opacity"
        style={{ backgroundColor: current.color + '18', color: current.color }}
      >
        {current.label}
        {secondary && <span className="text-[10px] opacity-70">+1</span>}
        <ChevronDown size={11} />
      </button>
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute z-20 left-0 top-full mt-1 w-56 bg-white dark:bg-surface-900 border border-subtle rounded-xl shadow-xl overflow-hidden"
            >
...
const InviteModal: React.FC<InviteModalProps> = ({ onClose, onInvited }) => {
  ...
  const handleSubmit = async () => {
    ...
    const roles = [...selectedRoles] as UserRole[];
    const role = primaryRole(roles);
    const secondaryRole = roles.find((r) => r !== role) ?? null;
    setLoading(true);
    try {
      await apiClient.post('/auth/register', { ...form, role, secondaryRole });
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-gray-500 text-center mb-6">
            ...
            lose access to Nexus immediately.
          </p>
          <div className="flex gap-2">
...
const UserManagementPage: React.FC = () => {
  ...
  const handleRoleUpdate = async (userId: string, role: UserRole, secondaryRole: UserRole | null) => {
    try {
      await apiClient.put(`/users/${userId}/role`, { role, secondaryRole });
      setUsers((p) => p.map((u) => (u.id === userId ? { ...u, role, secondaryRole: secondaryRole ?? undefined } : u)));
  ...
  const filtered = users.filter((u) => {
    ...
    const matchRole = !roleFilter || u.role === roleFilter || u.secondaryRole === roleFilter;
  ...
  const stats = [
    ...
    { label: 'Admins',   value: users.filter((u) => u.role === 'admin' || u.secondaryRole === 'admin').length,          color: '#f59e0b' },
    { label: 'Managers', value: users.filter((u) => u.role === 'manager' || u.secondaryRole === 'manager').length,      color: '#3b82f6' },
  ];
            ...
            {isLoading ? (
              ...
              <>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-subtle bg-surface-50/60 dark:bg-surface-800/40">
                      {['User', 'Roles', 'Status', 'Job Title', 'Actions'].map((h) => (
                        <th key={h} className="text-left px-5 py-3 text-[11px] font-bold uppercase tracking-wide text-gray-400">
                          {h}
                        </th>
                      ))}
                    </tr>
                    ...
                    <tbody>
                      <AnimatePresence initial={false}>
                        {filtered.map((u, i) => (
                          ...
                          >
                            <td className="px-5 py-3.5 w-[220px]">
                            ...
                            <td className="px-5 py-3.5 w-[170px]">
                              <RoleSelect
                                value={u.role}
                                secondary={u.secondaryRole ?? null}
                                onChange={(r, sec) => handleRoleUpdate(u.id, r, sec)}
                              />
                            </td>
